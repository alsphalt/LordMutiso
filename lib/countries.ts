/**
 * Country data for Darknote accounts.
 *
 * `User.country` stores the ISO 3166-1 alpha-2 code (e.g. "KE", "TZ", "US").
 * Flags are derived from the code using regional-indicator symbols, so the
 * emoji always matches the code — no hand-maintained flag table to drift.
 */
...
/** Flag emoji for an ISO alpha-2 code ("" when invalid). */
...
/** Convenience: flag + name, e.g. "🇰🇪 Kenya" ("" when unknown). */
...
export function isValidCountry(code?: string | null): boolean {
  return !!code && COUNTRIES.some((c) => c.code === code.toUpperCase());
}

/** Full country list (searchable picker source), sorted by name. */
export const COUNTRIES: CountryInfo[] = [
  ...
  { code: "KE", name: "Kenya" }, { code: "KI", name: "Kiribati" }, { code: "XK", name: "Kosovo" },
